...
class RealSensePCSubscriber(Node):
    ...
    def pointcloud_callback(self, msg: PointCloud2):
        ...
        mask_dist = dists <= float(self.max_distance)
        plane_vals = (
            self.a * points[:, 0] + self.b * points[:, 1] + self.c * points[:, 2] + self.d
        )
        mask_plane = plane_vals <= 0.0
        keep = mask_dist & mask_plane
        filtered_points = points[keep].astype(np.float32)
        ...
        if filtered_points.shape[0] > 0:
            cube_x, cube_y, cube_z = filtered_points.mean(axis=0)
        else:
            cube_x = 0.0
            cube_y = 0.0
            cube_z = 0.0
